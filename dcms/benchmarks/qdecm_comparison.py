"""qDECM solver comparison benchmark — Phase 5.

Generates test networks using the Chung-Lu power-law model (``k_s_generator_pl``),
then runs all applicable qDECM two-step solvers and prints a comparison table.
...
Usage::
    ...
    # Single network
    python -m src.benchmarks.qdecm_comparison --n 100 --seed 42

    # Multi-seed comparison (Phase 5 full run)
    python -m src.benchmarks.qdecm_comparison --n 1000 --n_seeds 10

    # Both N=1k and N=5k
    python -m src.benchmarks.qdecm_comparison --sizes 1000 5000

    # Fast mode: Newton / L-BFGS only (skip FP-GS Anderson which never converges for qDECM)
    python -m src.benchmarks.qdecm_comparison --n 1000 --fast

    # Phase 5 focused benchmark (N=1k, 10 seeds, saves bad seeds)
    python -m src.benchmarks.qdecm_comparison --phase5
...
from dcms.models.qdecm import qDECMModel, _ETA_MIN, _ETA_MAX, _LARGE_N_THRESHOLD as _DAECM_LARGE_N
...
def run_comparison(
    ...
    """Run all qDECM weight solvers on a single random network.
    ...
    """
    print(f"\n{'='*100}")
    print(f"qDECM Solver Comparison  |  N={N} nodes  |  seed={seed}  |  tol={tol:.0e}")
    print(f"{'='*100}")
...
def _run_single_network(
    ...
    """Run all qDECM weight solvers on one network realisation.
...
def run_multi_seed_comparison(
    ...
    """Run all qDECM weight solvers on *n_seeds* independent network realisations.
    ...
    if verbose:
        print(f"\n{'='*100}")
        print(
            f"qDECM Multi-Seed Comparison  |  N={N:,} nodes  |  "
...
def run_scaling_comparison(
    ...
    """Run multi-seed qDECM comparison for each size in *sizes*.
    ...
    print(f"\n{'='*74}")
    print(f"{'qDECM SCALING SUMMARY — Convergence Rate':^74}")
    print(f"{'='*74}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="qDECM solver comparison benchmark (Phase 5)."
    )
