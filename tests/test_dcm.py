...
Tests cover:
- pij_matrix: correct shape, zero diagonal, symmetry properties.
- residual:   correct value at the true solution.
- hessian_diag: all entries negative (Hessian of maximisation problem).
- jacobian:   symmetry check and consistency with finite differences.
...
def make_simple_model(N: int = 4, seed: int = 0) -> tuple[DCMModel, np.ndarray]:
    """Return a small DCMModel and a synthetic θ vector."""
...
class TestResidual:
    def test_zero_at_true_solution(self) -> None:
        ...
        assert np.max(np.abs(F)) < 1e-10, f"Max residual: {np.max(np.abs(F)):.3e}"
...
class TestGradient:
    def test_gradient_is_negative_residual(self) -> None:
        model, theta = make_simple_model(N=6)
        grad = model.gradient(theta)
        F = model.residual(theta)
        assert np.allclose(grad, -F)
...
class TestHessianDiag:
    def test_all_negative(self) -> None:
        ...
        assert np.all(h < 0), f"Some Hessian diag entries non-negative: {h[h >= 0]}"
...
class TestJacobian:
    ...
    def test_negative_diagonal(self) -> None:
        ...
        J = model.jacobian(theta)
        diag = np.diag(J)
        assert np.all(diag < 0), f"Some Jacobian diag entries non-negative: {diag[diag >= 0]}"
    ...
    def test_finite_difference_consistency(self) -> None:
        ...
        for j in range(n2):
            dth = np.zeros(n2)
            dth[j] = eps
            J_fd[:, j] = (model.residual(theta + dth) - model.residual(theta - dth)) / (2 * eps)
        assert np.allclose(J_exact, J_fd, atol=1e-6), (
            f"Max FD error: {np.max(np.abs(J_exact - J_fd)):.3e}"
        )
